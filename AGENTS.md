...
### Approved KEMI local credential-transfer exception

At the user's explicit direction, `app-k9mail` may transfer an email provider-generated app password as plaintext over
an HTTP connection from a phone browser to the tablet, but only for the KEMI phone-assisted account setup flow and
only over a private local IPv4 network. This exception does not apply to primary email passwords or any other feature.

The local receiver MUST bind to the selected private interface on a random port, use a high-entropy one-time write
token, accept at most one credential, expire within five minutes, enforce strict request and credential size limits,
avoid logging request paths, tokens, account identifiers, or credentials, and close immediately after success,
cancellation, or expiry. The UI MUST disclose that HTTP is unencrypted and instruct users to use a trusted local
network. The existing encrypted relay implementation and server deployment MUST remain unchanged so the client can be
switched back without a server deployment.
