...
class ThemeManager(
    ...
    private val displayCoreSettings: DisplayCoreSettings
        get() = displayCoreSettingsPreferenceManager.getConfig()

    override val appTheme: Theme
        get() = when (displayCoreSettings.appTheme) {
    ...
    override val messageViewTheme: Theme
        get() = resolveTheme(displayCoreSettings.messageViewTheme)

    override val messageComposeTheme: Theme
        get() = resolveTheme(displayCoreSettings.messageComposeTheme)
    ...
    override val messageViewThemeResourceId: Int
        get() = getSubThemeResourceId(displayCoreSettings.messageViewTheme)

    @get:StyleRes
    override val messageComposeThemeResourceId: Int
        get() = getSubThemeResourceId(displayCoreSettings.messageComposeTheme)
    ...
    fun init() {
        displayCoreSettingsPreferenceManager.getConfigFlow()
            .map { it.appTheme }
    ...
    fun toggleMessageViewTheme() {
        if (messageViewTheme === Theme.DARK) {
            displayCoreSettingsPreferenceManager.update { settings ->
                settings.copy(messageViewTheme = SubTheme.LIGHT)
            }
        } else {
            displayCoreSettingsPreferenceManager.update { settings ->
                settings.copy(messageViewTheme = SubTheme.DARK)
            }
        }
    }
