...
import com.fsck.k9.ui.AnchoredPopupMenuController
...
internal val MESSAGE_LIST_OVERFLOW_ACTION_IDS = listOf(
    R.id.select_all,
    ...
    R.id.debug_invalidate_access_token_local,
    R.id.debug_invalidate_access_token_server,
    R.id.debug_force_auth_failure,
    R.id.debug_feature_flags,
)

internal val MESSAGE_LIST_CONTEXT_OVERFLOW_ACTION_IDS = listOf(
    R.id.move,
    R.id.move_to_drafts,
    R.id.copy,
    R.id.flag,
...
class LegacyMessageListFragment :
    ...
    private val popupMenuController by lazy {
        AnchoredPopupMenuController(
            fragment = this,
            featureThemeProvider = featureThemeProvider,
            onMenuItemSelected = ::selectMenuItem,
        )
    }
        ...
        menuHost.addMenuProvider(
            object : MenuProvider {
                ...
                override fun onPrepareMenu(menu: Menu) {
                    ...
                    popupMenuController.prepareOverflowMenu(menu, MESSAGE_LIST_OVERFLOW_ACTION_IDS)
                }
    ...
    private fun enableFloatingActionButton(view: View) {
        ...
        (activity as? MessageHomeActivity)?.configureDualScreenComposeFloatingButton(floatingActionButton)
    ...
    private fun prepareSortMenu(menu: Menu) {
        ...
        popupMenuController.prepareSubMenuAction(
            menu = menu,
            itemId = R.id.set_sort,
            icon = DesignSystemR.drawable.ic_sort,
        )
    }
    ...
    private fun selectMenuItem(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.close_mail -> fragmentListener.closeMail()
    ...
    private fun setMessageList(messageListInfo: MessageListInfo) {
        val isInitialLoad = initialMessageListLoad
        ...
        if (isInitialLoad) {
            fragmentListener.onMessageListReady()
        }
    ...
    internal inner class ActionModeCallback : ActionMode.Callback {
        ...
        override fun onPrepareActionMode(mode: ActionMode, menu: Menu): Boolean {
            ...
            popupMenuController.prepareOverflowMenu(
                menu = menu,
                overflowActionIds = MESSAGE_LIST_CONTEXT_OVERFLOW_ACTION_IDS,
                onActionSelected = { item -> onActionItemClicked(mode, item) },
            )
