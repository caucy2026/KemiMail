...
import com.fsck.k9.activity.DualScreenPopupMenu
...
class MessageDetailsFragment : ToolbarBottomSheetDialogFragment() {
    ...
    private fun showOverflowMenu(view: View, participant: Participant) {
        ...
        val onMenuItemClickListener = PopupMenu.OnMenuItemClickListener { item: MenuItem ->
            onOverflowMenuItemClick(item.itemId, participant)
            true
        }
        popupMenu.setOnMenuItemClickListener(onMenuItemClickListener)

        if (!DualScreenPopupMenu.showIfNeeded(view, menu, onMenuItemClickListener)) {
            popupMenu.show()
        }
    }
