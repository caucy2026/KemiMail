...
object K9 : KoinComponent {
    ...
    fun init(context: Context) {
        ...
        trace("K9.checkCachedDatabaseVersion") {
            checkCachedDatabaseVersion(context)
        }

        val storage = trace("K9.resolveSettingsStorage") {
            Preferences.getPreferences().storage
        }
        trace("K9.loadPreferences") {
            loadPrefs(storage)
        }
    }
