...
internal class NotificationOperations(
    private val notificationController: Lazy<NotificationController>,
    ...
    private fun clearUnifiedFoldersNotifications() {
        for (account in accountManager.getAccounts()) {
            ...
            if (folderIds.isNotEmpty()) {
                notificationController.value.clearNewMailNotifications(account) { messageReferences ->
    ...
    private fun clearAllNotifications() {
        for (account in accountManager.getAccounts()) {
            notificationController.value.clearNewMailNotifications(account, clearNewMessageState = false)
        }
    }
