...
public class MessagingController implements MessagingControllerRegistry, MessagingControllerMailChecker {
    ...
    private final Lazy<NotificationStrategy> notificationStrategy;
    ...
    MessagingController(
        ...
        this(
                context,
                LazyKt.lazyOf(notificationController),
                LazyKt.lazyOf(notificationStrategy),
                localStoreProvider,
                backendManager,
                preferences,
                messageStoreManager,
    ...
    MessagingController(
        Context context,
        Lazy<NotificationController> notificationController,
        Lazy<NotificationStrategy> notificationStrategy,
        LocalStoreProvider localStoreProvider,
        BackendManager backendManager,
        Preferences preferences,
        MessageStoreManager messageStoreManager,
    ...
    public void handleAuthenticationFailure(LegacyAccountDto account, boolean incoming) {
        ...
        if (FeatureFlagProviderCompat.provide(featureFlagProvider,
            ...
            notificationController.getValue().showAuthenticationErrorNotification(account, incoming);
        }
    }
    ...
    private void cancelNotificationsForMessages(LegacyAccountDto account, long folderId, List<String> uids) {
        for (String uid : uids) {
            ...
            notificationController.getValue().removeNewMailNotification(account, messageReference);
        }
    }
    ...
    public void markMessageAsOpened(LegacyAccountDto account, LocalMessage message) {
        threadPool.execute(() ->
            notificationController.getValue().removeNewMailNotification(account, message.makeMessageReference())
        );
    ...
    private void showSendingNotificationIfNecessary(LegacyAccountDto account) {
        if (account.isNotifySync()) {
            notificationController.getValue().showSendingNotification(account);
        }
    }

    private void clearSendingNotificationIfNecessary(LegacyAccountDto account) {
        if (account.isNotifySync()) {
            notificationController.getValue().clearSendingNotification(account);
        }
    }
    ...
    protected void sendPendingMessagesSynchronous(final LegacyAccountDto account) {
        ...
        try {
            ...
            if (lastFailure != null) {
                notificationController.getValue().showSendFailedNotification(account, lastFailure);
            }
        } catch (Exception e) {
            Log.v(e, "Failed to send pending messages");
        } finally {
            if (lastFailure == null) {
                notificationController.getValue().clearSendFailedNotification(account);
            }
        }
    }
    ...
    private void showFetchingMailNotificationIfNecessary(LegacyAccountDto account, LocalFolder folder) {
        if (account.isNotifySync()) {
            notificationController.getValue().showFetchingMailNotification(account, folder);
        }
    }

    private void showEmptyFetchingMailNotificationIfNecessary(LegacyAccountDto account) {
        if (account.isNotifySync()) {
            notificationController.getValue().showEmptyFetchingMailNotification(account);
        }
    }

    private void clearFetchingMailNotification(LegacyAccountDto account) {
        notificationController.getValue().clearFetchingMailNotification(account);
    }
    ...
    public void deleteAccount(LegacyAccountDto account) {
        notificationController.getValue().clearNewMailNotifications(account, false);
    ...
    public void cancelNotificationsForAccount(LegacyAccountDto account) {
        notificationController.getValue().clearNewMailNotifications(account, true);
    }
    ...
    public void clearCertificateErrorNotifications(LegacyAccountDto account, boolean incoming) {
        notificationController.getValue().clearCertificateErrorNotifications(account, incoming);
    }
    ...
    public void notifyUserIfCertificateProblem(LegacyAccountDto account, Exception exception, boolean incoming) {
        if (exception instanceof CertificateValidationException) {
            notificationController.getValue().showCertificateErrorNotification(account, incoming);
        }
    }
    ...
    class ControllerSyncListener implements SyncListener {
        ...
        public void syncAuthenticationSuccess() {
            ...
            notificationController.getValue().clearAuthenticationErrorNotification(account, true);
        }
        ...
        public void syncNewMessage(@NotNull String folderServerId, @NotNull String messageServerId,
            ...
            if (!suppressNotifications &&
                    notificationStrategy.getValue().shouldNotifyForMessage(account, localFolder, message, isOldMessage)) {
                // Notify with the localMessage so that we don't have to recalculate the content preview.
                boolean silent = notificationState.wasNotified();
                notificationController.getValue().addNewMailNotification(account, message, silent);
        ...
        public void syncFlagChanged(@NotNull String folderServerId, @NotNull String messageServerId) {
            ...
            if (message.isSet(Flag.DELETED) || isMessageSuppressed(message)) {
                ...
                LocalFolder localFolder = message.getFolder();
                if (notificationStrategy.getValue().shouldNotifyForMessage(account, localFolder, message, false)) {
