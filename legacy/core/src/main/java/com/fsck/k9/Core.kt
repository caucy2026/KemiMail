...
object Core : KoinComponent {
    ...
    fun init(context: Context) {
        ...
        updateComponentsEnabled(context)
    ...
    fun setServicesEnabled(context: Context) {
        ...
        val enable = preferences.getAccounts().isNotEmpty()

        updateComponentsEnabled(appContext, enable)

        if (enable) {
            jobManager.scheduleAllMailJobs()
        } else {
            jobManager.cancelAllMailJobs()
        }
    }
    ...
    fun cancelMailSync(account: LegacyAccountDto) {
        jobManager.cancelMailSync(account)
    }

    private fun updateComponentsEnabled(context: Context) {
        val enabled = preferences.getAccounts().isNotEmpty()
        updateComponentsEnabled(context.applicationContext, enabled)
    }

    private fun updateComponentsEnabled(context: Context, enabled: Boolean) {
