...
class AccountOAuthViewModelTest {
    ...
    @Test
    fun `should expose native device authorization support on initState`() = runMviTest {
        val testSubject = createTestSubject(isDeviceAuthorizationSupported = true)
        val turbines = turbinesWithInitialStateCheck(testSubject, State())

        testSubject.initState(defaultState)

        assertThat(turbines.stateTurbine.awaitItem()).isEqualTo(
            defaultState.copy(isDeviceAuthorizationSupported = true),
        )
    }

    @Test
    fun `should show and cancel native device authorization code`() = runMviTest {
        val initialState = defaultState.copy(isDeviceAuthorizationSupported = true)
        val session = deviceAuthorizationSession()
        val testSubject = createTestSubject(
            startDeviceAuthorizationResult = StartDeviceAuthorizationResult.Success(session),
            initialState = initialState,
        )
        val turbines = turbinesWithInitialStateCheck(testSubject, initialState)

        testSubject.event(Event.DeviceSignInClicked)

        assertThat(turbines.stateTurbine.awaitItem()).isEqualTo(initialState.copy(isLoading = true))
        assertThat(turbines.stateTurbine.awaitItem()).isEqualTo(
            initialState.copy(
                deviceAuthorization = AccountOAuthContract.DeviceAuthorization(
        ...
        testSubject.event(Event.CancelDeviceSignInClicked)

        assertThat(turbines.stateTurbine.awaitItem()).isEqualTo(initialState)
    }

    @Test
    fun `should show generic error when native device authorization cannot start`() = runMviTest {
        val initialState = defaultState.copy(isDeviceAuthorizationSupported = true)
        val testSubject = createTestSubject(
            startDeviceAuthorizationResult = StartDeviceAuthorizationResult.Failure,
            initialState = initialState,
        )
        val turbines = turbinesWithInitialStateCheck(testSubject, initialState)
        ...
        assertThat(turbines.stateTurbine.awaitItem()).isEqualTo(initialState.copy(isLoading = true))
        assertThat(turbines.stateTurbine.awaitItem()).isEqualTo(
            initialState.copy(error = Error.DeviceAuthorizationFailed),
        )
    }
    ...
    private companion object {
        ...
        fun createTestSubject(
            ...
            pollDeviceAuthorizationResult: PollDeviceAuthorizationResult = PollDeviceAuthorizationResult.Failure,
            ...
            checkIsDeviceAuthorizationSupported = { isDeviceAuthorizationSupported },
            startDeviceAuthorization = { startDeviceAuthorizationResult },
            pollDeviceAuthorization = { pollDeviceAuthorizationResult },
            clock = { NOW },
            initialState = initialState,
        )

        fun deviceAuthorizationSession() = DeviceAuthorizationSession(
            configuration = OAuthConfiguration(
                ...
                deviceAuthorizationEndpoint = "https://example.com/devicecode",
            ),
