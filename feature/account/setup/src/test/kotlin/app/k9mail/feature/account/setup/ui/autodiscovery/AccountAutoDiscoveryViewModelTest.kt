...
class AccountAutoDiscoveryViewModelTest {
    ...
    @Test
    fun `should cancel device sign in and show password when OnUsePasswordClicked is received`() = runMviTest {
        val initialState = State(
            configStep = ConfigStep.OAUTH,
            emailAddress = StringInputField(value = "email"),
            isNextButtonVisible = false,
        )
        val oAuthViewModel = FakeAccountOAuthViewModel()
        val testSubject = createTestSubject(initialState, oAuthViewModel = oAuthViewModel)

        eventStateTest(
            viewModel = testSubject,
            initialState = initialState,
            event = Event.OnUsePasswordClicked,
            expectedState = initialState.copy(
                configStep = ConfigStep.PASSWORD,
                isNextButtonVisible = true,
            ),
        )
        assertThat(oAuthViewModel.events).isEqualTo(
            listOf(AccountOAuthContract.Event.CancelDeviceSignInClicked),
        )
    }

    @Test
    fun `should show remote password step when OnRemoteCredentialTransferClicked is received`() = runMviTest {
        val initialState = State(
            configStep = ConfigStep.PASSWORD,
            emailAddress = StringInputField(value = "email"),
        )
        ...
        eventStateTest(
            ...
            initialState = initialState,
            event = Event.OnRemoteCredentialTransferClicked,
            expectedState = initialState.copy(
                configStep = ConfigStep.REMOTE_PASSWORD,
                isNextButtonVisible = false,
            ),
        )
    }
    ...
    @Test
    fun `should fill password and return to password step when remote credential is received`() = runMviTest {
        val initialState = State(
            configStep = ConfigStep.REMOTE_PASSWORD,
            emailAddress = StringInputField(value = "email"),
            isNextButtonVisible = false,
        ...
        eventStateTest(
            viewModel = createTestSubject(initialState),
            initialState = initialState,
            event = Event.OnRemoteCredentialReceived("app-password"),
            expectedState = initialState.copy(
                configStep = ConfigStep.PASSWORD,
                password = StringInputField(value = "app-password"),
                isNextButtonVisible = true,
            ),
        )
    }
    ...
    fun `should change state to password when OnNextClicked event is received, input valid and discovery loaded`() =
        runMviTest {
            ...
            val testSubject = AccountAutoDiscoveryViewModel(
                ...
                remoteCredentialTransferViewModel = FakeRemoteCredentialTransferViewModel(),
    ...
    private companion object {
        fun createTestSubject(
            ...
            oAuthViewModel: FakeAccountOAuthViewModel = FakeAccountOAuthViewModel(),
