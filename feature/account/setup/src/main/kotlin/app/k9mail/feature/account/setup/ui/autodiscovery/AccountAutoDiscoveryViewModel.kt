...
internal class AccountAutoDiscoveryViewModel(
    ...
    override val remoteCredentialTransferViewModel: RemoteCredentialTransferContract.ViewModel,
    ...
    override fun event(event: Event) {
        when (event) {
            ...
            is Event.OnRemoteCredentialReceived -> onRemoteCredentialReceived(event.credential)
            ...
            Event.OnUsePasswordClicked -> showPasswordInput()
            Event.OnRemoteCredentialTransferClicked -> showRemoteCredentialTransfer()
            Event.OnRemoteCredentialTransferCanceled -> showPasswordInput()
        }
    }
    ...
    private fun onBack() {
        when (state.value.configStep) {
            ...
            -> {
                if (state.value.configStep == ConfigStep.OAUTH) {
                    oAuthViewModel.event(AccountOAuthContract.Event.CancelDeviceSignInClicked)
                }
                updateState {
                    it.copy(
            ...
            ConfigStep.REMOTE_PASSWORD -> {
                remoteCredentialTransferViewModel.event(RemoteCredentialTransferContract.Event.Cancel)
                showPasswordInput()
            }
        }
    }
    ...
    private fun showRemoteCredentialTransfer() {
        updateState {
            it.copy(
    ...
    private fun onRemoteCredentialReceived(credential: String) {
        updateState {
            it.copy(
                configStep = ConfigStep.PASSWORD,
                password = it.password.updateValue(credential),
                isNextButtonVisible = true,
            )
        }
    }
