...
abstract class BaseApplication : Application(), WorkManagerConfiguration.Provider {
    ...
    override fun onCreate() {
        trace("BaseApplication.onCreate") {
            trace("BaseApplication.superOnCreate") {
                super.onCreate()
            }

            trace("BaseApplication.initializeCore") {
                trace("BaseApplication.initializeK9") {
                    K9.init(this)
                }
                trace("BaseApplication.initializeLegacyCore") {
                    Core.init(this)
                }
            }
            trace("BaseApplication.initializeAppLanguage") {
                initializeAppLanguage()
            }
            trace("BaseApplication.observeNotificationChannels") {
                updateNotificationChannelsOnAppLanguageChanges()
            }
            trace("BaseApplication.initializeTheme") {
                themeManager.init()
            }
            appCoroutineScope.launch(Dispatchers.Default) {
                trace("BaseApplication.initializeMessageListWidget") {
                    messageListWidgetManager.init()
                }
            }

            trace("BaseApplication.registerMessagingListeners") {
                messagingListenerProvider.listeners.forEach { listener ->
                    messagingController.addListener(listener)
                }
            }
            trace("BaseApplication.observeProcessLifecycle") {
                ProcessLifecycleOwner.get().lifecycle.addObserver(
                    LoggerLifecycleObserver(debugLogConfigurator::getActiveSyncDebugFileLogSink),
                )
            }
        }
    }
